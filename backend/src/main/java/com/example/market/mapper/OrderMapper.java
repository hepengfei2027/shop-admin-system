...
public interface OrderMapper {
    ...
    @Insert("INSERT INTO orders(goods_id, buyer_id, seller_id, amount, original_amount, discount, coupon_id, coupon_amount, status, address_id, quantity, create_time, promotion_type, promotion_id, promotion_discount, group_status, goods_price, freight)"+
            "VALUES(#{goodsId}, #{buyerId}, #{sellerId}, #{amount}, #{originalAmount}, #{discount}, #{couponId}, #{couponAmount}, #{status}, #{addressId}, #{quantity}, NOW(), #{promotionType}, #{promotionId}, #{promotionDiscount}, #{groupStatus}, #{goodsPrice}, #{freight})")
